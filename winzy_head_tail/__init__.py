...
def create_parser_head(subparser):
    parser = subparser.add_parser("head", description="Mimics head command using python")
    parser.add_argument("files", metavar="FILE", nargs="*", help="File(s) to process. Reads from stdin if none provided.")
    parser.add_argument("-n", type=int, help="Number of lines to display (default: 10).")
    parser.add_argument("-c", type=int, help="Number of bytes to display (overrides -n).")
    return parser


def create_parser_tail(subparser):
    parser = subparser.add_parser("tail", description="Mimics tail command using python")
    parser.add_argument("files", metavar="FILE", nargs="*", help="File(s) to process. Reads from stdin if none provided.")
    parser.add_argument("-n", type=int, help="Number of lines to display (default: 10).")
    parser.add_argument("-c", type=int, help="Number of bytes to display (overrides -n).")
    return parser
